...
.controller("storiesController", function ($scope, $routeParams, $location, $http, $ionicPlatform, $ionicSideMenuDelegate, $ionicPopup, $ionicScrollDelegate, appData, appDataProcess, $ionicModal, $sce){
	...
	$scope.openStoryGetData =  function (isOpenMenuLeft, isOpenMenuRight, isNotFavoriteStory){
		...
		$scope.dataSchedule.data = "Loading...";
		var url = 'worldoftales/' + $scope.dataSchedule.link;
		$.ajax({
			url: url,
			type: 'GET',
			timeout: 10000,
			error: function(){
				$ionicPopup.alert({
					 title: 'Sorry!',
					 template: "Error when getting data."
				   });
				$scope.dataSchedule.data = "Error when getting data.";
			...
			success: function(data){
				if(data != ''){
					var content = $.trim(data).split("<p>").join("");
					content = content.split("</p>").join("<br/><br/>");
					content = content.split("\n").join("<br/><br/>");
					content = content.split("<br/>").filter(function(v)
															{ 
																var ct = v.trim();
																if(ct == "" || ct == "<br>" || ct == "<br/>" ) return false;
																return true;
															}).join("<br/><br/>");

					$scope.dataSchedule.data = content;//data.replace(/\t\n/g,"<br/>");//$('#schedule_content').html(jsondata);
					appDataProcess.saveLastId($scope.category.id, $scope.dataSchedule.id);
				}
				else{
					$scope.dataSchedule.data = $('<div/>').html('<font style="width: 100%; padding: 15px; text-align: center; float: left; color: #E67D00">Nothing here</font>').html();
				}
				if(isOpenMenuLeft) $ionicSideMenuDelegate.toggleLeft(true);
				$ionicScrollDelegate.$getByHandle('ionScrollContentStory').scrollTop();
			}
    ...
    $scope.saveFavoriteStory = function(story) {
	  ...
	  $scope.removeFavoriteStory = function(story) {
    	...
    	for(var i =0; i<$scope.lstFavoriteStories.length; i++)
    	{
    		if($scope.lstFavoriteStories[i] == null) continue;
	  		if($scope.lstFavoriteStories[i].link == story.link)
	  		{ 
	  ...
	  $scope.wasFavoriteStory = function(story) {
	  	for(var i =0; i<$scope.lstFavoriteStories.length; i++)
	  	{
	  		if($scope.lstFavoriteStories[i] == null) continue;
	  		if($scope.lstFavoriteStories[i].link == story.link) return true;
	  	}
    	return false;
	  };
...
.run(function($ionicPlatform) {
  $ionicPlatform.ready(function() {
	...
	function removeTone(str){
      ...
      var ad_units = {
        android : {
          banner: 'ca-app-pub-4576847792571626/3918689997',
          interstitial:'ca-app-pub-4576847792571626/5893546798'
         }
      };

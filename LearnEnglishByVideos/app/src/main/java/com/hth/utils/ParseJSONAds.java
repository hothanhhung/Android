...
public class ParseJSONAds {
	...
	private void setServer1(String newDomain) {
		SharedPreferences appSharedPrefs = context.getSharedPreferences(APP_SHARED_PREFS, 0);
		SharedPreferences.Editor prefsEditor = appSharedPrefs.edit();
		prefsEditor.putString(RECODE_USERCLICKAD, newDomain);
		prefsEditor.commit();
	}

	private String getServer1()
	{
		SharedPreferences appSharedPrefs = context.getSharedPreferences(APP_SHARED_PREFS, 0);
		return appSharedPrefs.getString(RECODE_USERCLICKAD, SERVER_DEFAULT_1);
	}
	...
	private String getLinkServerUserClick(String link) {
		return getServer1() + "?country=" + country + "&os=" + os + "&info=" + link + "&device=" + getAndroidId() + "&open=" + openKey + "&version=" + getAppVersion() + "&package=" + getPackageName();
	}

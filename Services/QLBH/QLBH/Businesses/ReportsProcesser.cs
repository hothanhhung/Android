...
namespace QLBH.Businesses
{
    ...
    {
        public static List<ReportItem> GenerateReportOfProfit(int? productId, string from, string to, int group = 0, bool isIncludeFee = true)
        {
            ...
            var reportItemInTotal = new ReportItem()
            {
                ...
                Name = "KẾT QUẢ ",
                InCome = reportItems.Sum(r => r.InCome),
                OutCome = reportItems.Sum(r => r.OutCome),
                FeeForShip = reportItems.Sum(r => r.FeeForShip),
                Fee = reportItems.Sum(r => r.Fee),
            };
            reportItems = reportItems.OrderBy(r => r.DateForReport).ToList();
            reportItems.Insert(0, reportItemInTotal);
            return reportItems;
        }
    }
}

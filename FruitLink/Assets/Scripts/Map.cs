...
public class Map : MonoBehaviour 
{
	...
	private const int  MAX_NUMBER_ITEM = 25;
	...
	void CheckPair(Vec2 pos)
	{
		...
		if(!IsSameType(MAP[POS1.R][POS1.C], MAP[POS2.R][POS2.C]))
		{
	...
	private string GetItemPathFromType(int type)
	{
		var path = "Images/item/item";
		if (type < 100)
		{
			path = path + "" + type;
		}
		else{
			path = path + "" + (type/100) +"A";
		}
		
		return path;
	}
	
	private bool IsSameType(int type1, int type2)
	{
		if ((type1 == type2 * 100) || (type1 * 100 == type2 ))
			return true;
		else 
			return false;
	}
	...
	public void LMap(int row, int col)
	{
		...
		CELL_WIDH = CELL_HEIGHT;//(int)(CELL_HEIGHT * 0.9f);
	...
	public void CheckAndSwapThings(bool hasSound)
	{
		...
		bool changed = false;
		//LLSound.I.playWrong();
		while (CheckIsAvailable() == false && loopCount < 5000)
		{
			changed = true;
		...
		if(loopCount >= 5000)
		{
			throw new System.Exception("Too long for change map");
		}
	}
	Vec2 HINT_POS0;
	Vec2 HINT_POS1;
			...
			for (int j = 1; j < COL - 1; j++)
		{
			if (MAP[i][j] != -1)
			{
					...
					for (int j2 = 1; j2 < COL - 1; j2++)
				{
					...
					if (IsSameType(MAP[i2][j2], MAP[i][j]))
